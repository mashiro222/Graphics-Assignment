...
{
	// boundary 不是引用
	auto boundary = minSurf->GetLargestBoundary();
...
bool Ubpa::Paramaterize::GetTexcoods(EnumMethod weightmethod, EnumMethod boundarymethod, Eigen::MatrixX2f& texcoords)
{
	minSurf->GenConstraint(1);
	switch (weightmethod)
	{
	...
	minSurf->Minimize();
	size_t nV = minSurf->heMesh->NumVertices();
	texcoords.resize(nV, 2);
	for (int i = 0; i < nV; i++)
	{
		texcoords(i, 0) = minSurf->heMesh->Vertices()[i]->pos[0];
		texcoords(i, 1) = minSurf->heMesh->Vertices()[i]->pos[1];
	}
	//triMesh 和 heMesh 中的点索引一致， 所以对应赋值
	return true;
}
