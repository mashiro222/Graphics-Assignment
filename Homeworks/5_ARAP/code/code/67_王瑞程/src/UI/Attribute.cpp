...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	Ptr<Paramaterize::EnumMethod> pWeightMethod = std::make_shared<Paramaterize::EnumMethod>();
	*pWeightMethod = Paramaterize::EnumMethod::UNIFORM_WEIGHT;
	grid->AddComboBox("Weight Method", parameterizingWMethodStrs, [pWeightMethod](const std::string& str)
	{
		if (str == "Uniform")
			*pWeightMethod = Paramaterize::EnumMethod::UNIFORM_WEIGHT;
		else if (str == "Cotangent")
			*pWeightMethod = Paramaterize::EnumMethod::COTANGENT_WEIGHT;
		else if (str == "Shape preserving")
			*pWeightMethod = Paramaterize::EnumMethod::SHAPE_PRESERVING;
	});

	std::vector<std::string> parameterizingBMethodStrs = {"Round", "Square"};
	Ptr<Paramaterize::EnumMethod> pBoundaryMethod = std::make_shared<Paramaterize::EnumMethod>();
	*pBoundaryMethod = Paramaterize::EnumMethod::ROUND_BOUNDARY;
	grid->AddComboBox("Boundary Method", parameterizingBMethodStrs, [pBoundaryMethod](const std::string& str)
	{
		if (str == "Round")
			*pBoundaryMethod = Paramaterize::EnumMethod::ROUND_BOUNDARY;
		else if (str == "Square")
			*pBoundaryMethod = Paramaterize::EnumMethod::SQUARE_BOUNDARY;
	});

	grid->AddButton("Parameterize", [mesh, pOGLW = attr->pOGLW, pWeightMethod, pBoundaryMethod]() {
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->Run(*pWeightMethod, *pBoundaryMethod))
			printf("Parameterize done\n");
		pOGLW->DirtyVAO(mesh);
	});


	/*grid->AddButton("Para & Show mesh", [mesh, pOGLW = attr->pOGLW, pWeightMethod, pBoundaryMethod]() {
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->Run(*pWeightMethod, *pBoundaryMethod))
			printf("Parameterize done\n");
		paramaterize->GetMinSurfMesh();
		pOGLW->DirtyVAO(mesh);
	});*/
	//HW5:
	std::vector<std::string> localGlobalBMethodStrs = { "ASAP", "ARAP" ,"ASAP_once"};
	Ptr<Paramaterize::EnumMethod> pLocalGlobalMethod = std::make_shared<Paramaterize::EnumMethod>();
	*pLocalGlobalMethod = Paramaterize::EnumMethod::ASAP;
	grid->AddComboBox("Local/Global Method", localGlobalBMethodStrs, [pLocalGlobalMethod](const std::string& str)
	{
		if (str == "ASAP")
			*pLocalGlobalMethod = Paramaterize::EnumMethod::ASAP;
		else if (str == "ARAP")
			*pLocalGlobalMethod = Paramaterize::EnumMethod::ARAP;
		else if (str == "ASAP_once")
			*pLocalGlobalMethod = Paramaterize::EnumMethod::ASAP_ONCE;
	});

	Ptr<int> pIteration = std::make_shared<int>();
	*pIteration = 4;
	grid->AddEditVal(std::string("Iteration"), *pIteration, 0, 20);
	grid->AddButton("Do Local/Global", [mesh, pOGLW = attr->pOGLW, pLocalGlobalMethod ,pIteration]() {
		if (*pLocalGlobalMethod == Paramaterize::EnumMethod::ASAP)
		{
			auto asap = ASAP::New(mesh);
			if (asap->Run(*pIteration))
				printf("Parameterize done\n");
		}
		else if(*pLocalGlobalMethod == Paramaterize::EnumMethod::ARAP)
		{
			auto arap = ARAP::New(mesh);
			if (arap->Run(*pIteration))
			printf("Parameterize done\n");
		}
		else if (*pLocalGlobalMethod == Paramaterize::EnumMethod::ASAP_ONCE)
		{
			auto asap = ASAP::New(mesh);
			if (asap->RunOnce())
				printf("Parameterize done\n");
	...
	grid->AddButton("Flatten", [mesh, pOGLW = attr->pOGLW]() {
		auto flatten = LocalGlobalPara::New(mesh);
		flatten->Flatten();
		pOGLW->DirtyVAO(mesh);
	});
	//////////////
