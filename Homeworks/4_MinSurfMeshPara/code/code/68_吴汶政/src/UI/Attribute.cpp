...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	vector<string> weight_list_ = { "Uniform", "Cotangent" };
	auto WeightSlot = [=](const string& item) {
		cout << "Select weight : " << weight->currentIndex() << endl;
	};
	grid->AddComboBox(weight, "ParamaterizeWeight", weight_list_, WeightSlot);
	//------- End ------
	...
	grid->AddButton("ParamaterizeMesh", [mesh, pOGLW = attr->pOGLW, shape, weight]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->setShape(shape->currentIndex());
		paramaterize->setWeight(weight->currentIndex());
		if (paramaterize->Show())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});
