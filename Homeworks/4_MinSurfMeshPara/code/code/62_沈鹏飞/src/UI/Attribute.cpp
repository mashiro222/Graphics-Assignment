...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	grid->AddButton("Minimize Surface Using cotangent weight", [mesh, pOGLW = attr->pOGLW]() {
		auto minSurf = MinSurf::New(mesh);
		minSurf->Run(kCot);
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Set Texcoords", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->SetTexcoords())
			printf("Set Texcoords done\n");
		pOGLW->DirtyVAO(mesh);
	});
