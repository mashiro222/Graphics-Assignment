...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	grid->AddButton("Minimize Surface", [this, mesh, pOGLW = attr->pOGLW]() {
		auto minSurf = MinSurf::New(sobj, mesh);
		minSurf->Run();
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("ParamaterizeCircle", [this, mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(sobj, mesh, true, 0, 0);
		if (paramaterize->Run())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});
	grid->AddButton("ParamaterizeSquareNaive", [this, mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(sobj, mesh, true, 1, 0);
		if (paramaterize->Run())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});
	grid->AddButton("ParamaterizeSquareCotan", [this, mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(sobj, mesh, true, 1, 1);
		if (paramaterize->Run())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});
	grid->AddButton("TextureMappingNaive", [this, mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(sobj, mesh, false, 1, 0);
		if (paramaterize->Run())
			printf("texure mapping done\n");
		pOGLW->DirtyVAO(mesh);
	});
	grid->AddButton("TextureMappingCotan", [this, mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(sobj, mesh, false, 1, 1);
		if (paramaterize->Run())
			printf("texure mapping done\n");
		pOGLW->DirtyVAO(mesh);
	});
