...
bool Paramaterize::Init(Ptr<SObj> triMeshObj, Ptr<TriMesh> triMesh, bool is_flaten, int mode, int pType)
{
	Clear();
	this->mode = (mode == 0) ? Circle_boundary : Square_boundary;
	this->pType = (pType == 0) ? Naive : Cotan;
	this->is_flaten = is_flaten;
...
void Paramaterize::SetBoundaryVCoefficient()
{
	...
	{
		size_t idx = heMesh->Index(get<0>(reshaped_boundary_V_pos[i]));
	...
	if (Circle_boundary == mode)
	{
		for (size_t i = 0; i < boundary_V.size(); i++)
		{
			reshaped_boundary_V_pos.push_back(tuple<V*, pointf3>(
				boundary_V[i], 
				pointf3(0.5 * cos(mat_nE_boundary_len(i, 0)), 
						0.5 * sin(mat_nE_boundary_len(i, 0))
						,0)
				)
			);
...
void Paramaterize::Minimize(Parameterize_Type pType = Naive)
{
	...
	for (size_t i = 0; i < nV; i++)
	{
		if (is_flaten) { // if need to flaten to a plane
