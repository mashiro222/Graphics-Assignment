...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	grid->AddButton("Paramaterize Circle Uniform", [mesh, pOGLW = attr->pOGLW]() {
		...
		paramaterize->Set_Barycentric_Uniform();
		if (paramaterize->Run())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Paramaterize Circle Cot", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_Circle();
		paramaterize->Set_Barycentric_Cot();
		if (paramaterize->Run())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});
	...
	grid->AddButton("Paramaterize Square Cot", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_Square();
		paramaterize->Set_Barycentric_Cot();
		if (paramaterize->Run())
	...
	grid->AddButton("Paramaterize Circle Cot Display", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_Circle();
		paramaterize->Set_Barycentric_Cot();
		paramaterize->Set_Display();
	...
	grid->AddButton("Paramaterize Square Cot Display", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->Set_Boundary_Square();
		paramaterize->Set_Barycentric_Cot();
		paramaterize->Set_Display();
	...
	grid->AddButton("ASAP Display", [mesh, pOGLW = attr->pOGLW]() {
		auto ASAP = ASAP::New(mesh);
		ASAP->Set_Display();
		if (ASAP->Run())
			printf("ARAP done\n");
	...
	//------Add Value------
	int* time = new int(10);
	grid->AddEditVal({ "ARAPtimes" }, 1, 1, 10, [=](const int& val) {
		*time = val;
		});
	//------End------
	...
	grid->AddButton("ARAP Display", [mesh, pOGLW = attr->pOGLW, time]() {
		auto ARAP = ARAP::New(mesh);
		ARAP->setTimes(*time);
		ARAP->Set_Display();
		if (ARAP->Run())
