		...
		case kPaste:
			{
                ...
                if ( (xpos+w<image_.cols) && (ypos+h<image_.rows))
				{
					...
					case kCopy:
                        for (int i=0; i<w; i++)
                            for (int j=0; j<h; j++)
                                if (1 == source_window_->imagewidget_->in_points(xsourcepos+i, ysourcepos+j))
...
void ImageWidget::Open(QString filename)
{
	...
	if (!filename.isEmpty())
	{
        ...
        in_points.resize(image_.cols,image_.rows);
        in_points = Eigen::MatrixXi::Zero(image_.cols,image_.rows);
	}
...
void ImageWidget::find_inside_points()
{
	...
	case kPolygon:
		...
		in_points = scanline.insideMatrix();
		break;
    case kFree:
        scanline.init(ImageWidth(),ImageHeight(),polygon_);
        in_points = scanline.insideMatrix();
...
void ImageWidget::find_boundary()
{
	for (int i=0;i<ImageWidth();i++)
		for (int j=0;j<ImageHeight();j++)
			if (0 == in_points(i,j))
			{
				if (i+1<ImageWidth() && 1 == in_points(i+1,j))
					in_points(i,j) = 2;
				else if (i>0 && 1 == in_points(i-1,j))
					in_points(i,j) = 2;
				else if (j+1<ImageHeight() && 1 == in_points(i,j+1))
					in_points(i,j) = 2;
				else if (j>0 && 1 == in_points(i,j-1))
					in_points(i,j) = 2;
			}
}
...
void ImageWidget::ClearChosen()
{
	point_start_ = point_end_ = QPoint(0,0);
	polygon_.clear();
	in_points = Eigen::MatrixXi::Zero(ImageWidth(),ImageHeight());
}
