...
void MainWindow::CreateActions()
{
	...
	action_choose_ = new QAction(tr("RectChoose"), this);
	connect(action_choose_, SIGNAL(triggered()), this, SLOT(ChooseRect()));
	...
	connect(action_paste_, SIGNAL(triggered()), this, SLOT(PasteNormal()));

	action_seamless_ = new QAction(tr("Seamless"), this);
	action_seamless_->setStatusTip(tr("Paste using seamless cloning"));
	connect(action_seamless_, SIGNAL(triggered()), this, SLOT(PasteSeamless()));

	action_mixed_ = new QAction(tr("Mixed"), this);
	action_mixed_->setStatusTip(tr("Past using mixed seamless cloning"));
	connect(action_mixed_, SIGNAL(triggered()), this, SLOT(PasteMixed()));
}
...
void MainWindow::CreateToolBars()
{
	...
	toolbar_file_->addAction(action_mixed_);
}
...
void MainWindow::ChooseRect()
{
	...
	window->imagewidget_->set_choose_status_to_rect();
...
void MainWindow::ChoosePoly()
{
	...
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_choose();
	window->imagewidget_->set_choose_status_to_polygon();
	child_source_ = window;
}
...
void MainWindow::ChooseFree()
{
	...
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_choose();
	window->imagewidget_->set_choose_status_to_freehand();
	child_source_ = window;
}

void MainWindow::PasteNormal()
{
	// Paste image region to object image
	ChildWindow* window = GetChildWindow();
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_paste();
	window->imagewidget_->set_clone_status_to_default();
...
void MainWindow::PasteSeamless()
{
	...
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_paste();
	window->imagewidget_->set_clone_status_to_seamless();
	window->imagewidget_->set_source_window(child_source_);
}
...
void MainWindow::PasteMixed()
{
	...
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_paste();
	window->imagewidget_->set_clone_status_to_mixed();
	window->imagewidget_->set_source_window(child_source_);
}
